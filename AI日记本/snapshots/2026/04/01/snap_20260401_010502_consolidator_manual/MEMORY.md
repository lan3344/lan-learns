...
| LAN-059 | lan_snapshot_consolidator.py | 记忆快照中速器 v1.0（恺江 2026-04-01）：压力感知（MEMORY≥200KB/日记≥300行）自动打快照 → 队列12合1生成代表快照（互相对比挑最重要）→ 月度集成（月底26日后自动跑）→ 睡眠模式（all_healthy时在自循环里静默运行）。数据藏在 AI日记本/.consolidator/（不打扰日常）。命令：check/take/consolidate/monthly/query/status/sleep-run |
